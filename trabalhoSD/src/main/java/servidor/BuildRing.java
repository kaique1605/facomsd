...
public class BuildRing {
  public void buildRing(int n, int m) {
    try {
      ...
      // System.out.println("reminder "+Constant.maxKey+" \\ "+n+": " +
      // Constant.maxKey.mod(BigInteger.valueOf(n)));
      ...
      for (int i = 0; i < n; i++) {
        ...
        t[i] = new Thread(new ThreadStartServer(Constant.HOST, porta, BigInteger.valueOf(i), minKey, maxKey, antecessor, sucessor));
