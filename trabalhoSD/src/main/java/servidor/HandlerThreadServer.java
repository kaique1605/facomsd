...
public class HandlerThreadServer extends Thread {
  ...
  public HandlerThreadServer(QueueCommand queueCommand, Request req, StreamObserver<Reply> responseObserver, Finger finger) {
    try {
      this.finger = finger;
    ...
    try {
      ...
      if (HandlerCommandClient.checkComand(comando)) {
        ...
        if (clientComand.getKey().compareTo(finger.getMinKey()) >= 0 && clientComand.getKey().compareTo(finger.getMaxKey()) <= 0) {
          ...
          if (finger.getPort() == finger.getAntecessor() || finger.getPort() == finger.getSucessor() || !isViable(clientComand)) {// é unico
                                                                                                                                  // no do
                                                                                                                                  // anel
  ...
  private boolean isViable(ClientData clientComand) {
    if (clientComand.getKey().compareTo(Constant.getMaxKey()) > 0 || clientComand.getKey().compareTo(Constant.getMinKey()) < 0) {
      return false;
    }
    return true;
  }
